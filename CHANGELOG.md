...
- Fixed the iTerm2 focus script, which closed its innermost `repeat` with `end tell` and so failed to compile. iTerm2 focus never worked in a released version; it was introduced broken in 3.1.0. The defect was invisible to every existing check because AppleScript is a string until something runs it, and a key press was the only thing that did. Two tests now guard it: a portable block-balance check, and `osacompile` on macOS, which reports syntax errors without executing the script.
