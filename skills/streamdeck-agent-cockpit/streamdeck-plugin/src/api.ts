...
import type { AgentCockpitSettings, ApiEnvelope, ControlState } from "./types";
...
export class AgentCockpitApi {
	...
	private async token(): Promise<string> {
		...
		if (token.length < 24) throw new Error("token file is empty or invalid");
		return token;
	}
	...
	control(controlId: string): Promise<ControlState> {
	...
	async invoke(
		...
		await this.request(`/v1/controls/${encodeURIComponent(controlId)}/invoke`, {
			method: "POST",
			body: JSON.stringify({ gesture, confirmed, value: 0 }),
		});
	}
}
