...
import { AgentCockpitApi } from "../api";
import { errorImage, imageFor } from "../render";
import type { AgentCockpitSettings } from "../types";

const DEFAULT_HOLD_MS = 650;
const DEFAULT_POLL_MS = 1500;

@action({ UUID: "com.cskwork.agent-cockpit.control" })
export class AgentCockpitAction extends SingletonAction<AgentCockpitSettings> {
	...
	override async onKeyUp(event: KeyUpEvent<AgentCockpitSettings>): Promise<void> {
		...
		try {
			await new AgentCockpitApi(settings).invoke(controlId, isHold ? "longPress" : "tap", isHold);
	...
	private startPolling(
		...
		this.pollers.set(context, setInterval(() => void this.refresh(actionInstance, settings), pollMs));
	}
	...
	private async refresh(
		...
		if (!controlId) {
			await actionInstance.setImage(errorImage("SET CONTROL"));
			return;
		}
		try {
